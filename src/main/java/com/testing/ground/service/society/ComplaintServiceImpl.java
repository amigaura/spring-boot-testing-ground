...
//        return allComplaints.isEmpty() ? Collections.emptyList() : allComplaints;
    ...
    public String escalateComplaint(String complaintId) {
        try {
            ...
            if (complaintOpt.isPresent()) {
                Complaint complaint = complaintOpt.get();
                complaint.setStatus(ComplaintStatus.valueOf("ESCALATED"));
                complaintRepository.save(complaint);
    ...
    public String archiveComplaint(String complaintId) {
        try {
            ...
            if (complaintOpt.isPresent()) {
                Complaint complaint = complaintOpt.get();
                complaint.setStatus(ComplaintStatus.valueOf("CLOSED"));
                complaintRepository.save(complaint);
    ...
    public String reopenComplaint(String complaintId) {
        try {
            ...
            if (complaintOpt.isPresent()) {
                Complaint complaint = complaintOpt.get();
                complaint.setStatus(ComplaintStatus.valueOf("REOPENED"));
                complaintRepository.save(complaint);
        ...
        try {
            ...
            if (complaintOpt.isPresent()) {
                ...
                if (assignTo == null || assignTo.isEmpty()) {
                    throw new IllegalArgumentException("Assign to user ID cannot be null or empty");
                }
                AppUser user = userRepository.findByUsername(assignTo).orElseThrow(() -> new RuntimeException("User being assigned to is not found"));;
                complaint.setAssignedToUserId(user.getId()); // Ensure Complaint entity has this field and setter
                complaintRepository.save(complaint);
